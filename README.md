...
当前分支已经完成 UI-03～UI-09 的主要迁移闭环：角色发现、资料、成果、个人背景、设置、隐私/备份，以及 Top 1 对话与资料/成果联动均已接入真实 Room/Repository。多角色默认独立回应、用户选择资料确认和成果来源追溯也已纳入对话链路。PR09-13B 的正式可移植导出、设备绑定快照、Argon2id、确定性 CBOR、Tink Streaming AEAD、完整 EOF 校验、快照闭库重开和全局并发门禁已经接入生产代码；Portable 导入与数据库替换仍明确属于后续 PR09-14A/14B。
...
UI-03～UI-09 的页面和主要用户闭环、PR09-13B 正式备份导出与设备快照已经进入本分支；后续不再把旧的“议题/工作区”迁移清单当作未完成实现。当前仍需单独完成并审查的发布门禁是：

1. 完成对应的外部安全审查、跨版本向量验证、受限设备性能和真实设备恢复验收；
2. 按 PR09-14A/14B 实现 Portable 隔离导入、差异预览、冲突确认和数据库原子替换；
3. 继续维护真实人物型角色的来源、授权与 AI 模拟身份边界。

在上述门禁关闭前，当前备份导出可以作为 V1 正式加密文件使用；导入/替换能力尚未开放，且不应把本地模拟器验收当作外部安全审查或多设备恢复结论。
