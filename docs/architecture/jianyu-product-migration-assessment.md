...
7. **PR08-F 批准前不启动 PR09**：本文中的表名、算法、文件格式和拆分均为候选，需要在最终规格中冻结后再实现。
...
由于目标 `applicationId` 将创建新的应用沙箱，且冻结为不迁移旧包数据，PR09 **不需要**为旧包数据库设计跨包 Migration。候选方案是在 `com.elio.jianyu` 下建立新的数据库身份和新的初始 Schema 版本；是否保留 `RoundtableDatabase` 类名、旧版本历史或改为 `JianyuDatabase`，必须由 PR08-F/PR09 数据 PR 明确决定。
...
2. ID 类型和稳定导出 ID 规则；
3. 资料、成果和个人背景的最终字段与删除语义；
4. Stage/Run/Message 的精确边界；
5. Skill 官方定义版本化格式；
6. 可移植导出 Envelope 与 KDF/AEAD 算法参数；
7. 快照目录、容量提醒阈值和保留 UI；
8. 文件切换采用同进程原子 rename 还是受控重启；
9. 音频成果的格式和生命周期；
10. 官网托管平台和发布渠道；
11. 仓库改名的精确发布日期；
12. PR09 最终编号、负责人和并行窗口。
