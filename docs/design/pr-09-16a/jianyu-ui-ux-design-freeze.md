...
| 多 Skill 独立回应 | 360dp 不将长回复硬分为多栏。先并列显示三位 Skill 的可选阵容条（短名、身份标记、状态），再在下方展示当前选中 Skill 的完整回应、AI 生成标识、短职责、来源/反馈操作；≥600dp 才可升为多列完整回应。 | 失败成员不清空成功消息；部分成功在状态轨给出完成/失败计数。 |
