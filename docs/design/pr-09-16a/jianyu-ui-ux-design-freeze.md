...
| 持续容器 | 议题 | 议题列表、首页最近议题、成果深链 | 保存阶段、资料、阵容和历史。 |
...
>
> 首页也可「仅保存议题」→ 议题列表与详情 → 工作区。底栏固定：首页、议题、Skill、资料与成果。工作区可打开阶段草稿/正式成果或推进议题三步面板；议题列表管理归档和回收站；设置由全局入口进入。

1. 启动页只展示必要初始化；无法使用时进入相应引导，不永久阻断本地内容访问。
2. 工作区、推荐确认、资料详情、成果详情、回收站、设置均是二级承载；系统返回等于顶部返回。
...
| 议题执行工作区 | 360dp 单列：标题/阶段、时间线入口、状态条、回应流、输入/操作栏；≥600dp 可有辅助栏，长文最大 720dp。 | 运行、部分成功、停止、恢复、限流、无 Key 均在运行/回应作用域内可读、可恢复。 |
| 多 Skill 独立回应 | 回应卡：Skill、职责、状态、来源范围、正文、原位重试。 | 失败成员不清空成功成员；部分成功显示完成/失败计数。 |
| 点名回应 | 输入上方可移除的定向对象，结果标「定向回应」。 | 完成后回到常规阵容；点名失败只重试该运行。 |
| 交叉讨论 | 明确分组显示参与者、焦点、整合者、成功/失败成员与综合状态。 | 不能显示成全体共识；可重试失败成员或综合步骤。 |
...
| 议题列表与详情 | 进行中、已归档、回收站三段；项显示标题、阶段、更新时间、运行/恢复、成果。 | 空/加载/失败/离线都保留区段标题，避免误认为内容被删除。 |
...
| TalkBack 顺序 | 页面标题 → 全局状态 → 当前阶段/关键摘要 → 内容列表 → 主操作 → 次操作 → 底栏；弹窗关闭后回触发点。 |
