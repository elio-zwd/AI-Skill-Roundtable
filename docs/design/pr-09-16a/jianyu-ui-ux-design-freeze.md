...
| 议题执行工作区 | Conversation-first：顶部工具栏只保留返回、短议题标题和功能菜单；紧随其后用一行紧凑上下文显示阶段与 Skill 数量，不重复显示「AI 对话工作区」或大标题。消息流是唯一主体；聊天输入器固定在底部安全区域，工作区不显示产品底栏。≥600dp 可有辅助栏，长文最大 720dp。 | 运行、部分成功、停止、恢复、限流、无 Key 都压缩为消息流上方的一行状态轨，不挤占对话高度。 |
| 多 Skill 独立回应 | 360dp 不将长回复硬分为多栏。使用文字式的三位 Skill 切换条（短名与身份标记，活动项仅用下划线），下方以无边框的编辑式观点正文展示当前选中 Skill；AI 生成、短职责、来源和状态降为轻量文字，不堆叠胶囊或嵌套卡片。≥600dp 才可升为多列完整回应。 | 失败成员不清空成功消息；部分成功在状态轨给出完成/失败计数。 |
...
| Skill 目录 | 内容优先的可用能力列表：顶部为单行搜索与类型筛选，结果以带分隔线的紧凑信息行呈现名称、类型、职责、可用性和边界；「查看详情」在原位展开适用范围与诚信/事实边界，不使用大面积嵌套卡片。 | 不可用 Skill 仍可发现，显示原因与替代/返回；无结果给出缩短关键词或切换「全部」的恢复路径。 |
...
| 区域 | 固定位置 | 内容与行为 |
|---|---|---|
| 顶部上下文条 | 工作区顶端：56dp 工具栏 + 紧凑上下文行。 | 工具栏只放返回、短议题标题、更多菜单；阶段和阵容数量放入下一行。标题过长省略但 TalkBack 读完整；不重复展示大标题、方向卡、阶段卡或「AI 对话工作区」。 |
