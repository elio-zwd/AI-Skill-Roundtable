...
## 第二次返工独立复核 — 2026-09-24 / commit `a769ed8feb7fc35b4c87e071e16340715fb986ad`

第二次返工后，以下 5 张已达到目标，可保留：
...
2. `charlie_munger.jpg`
   - 头发顶部左右出现矩形补块/阶梯状边缘。
   - 要求：恢复自然头发轮廓，背景连续，不得保留矩形块。

3. `feng_ge.jpg`
   - 头发左上/侧面出现明显矩形块状补边。
   - 要求：去掉矩形伪影，保持原头发自然外轮廓。

4. `ilya_sutskever.jpg`
   - 头发顶部被水平矩形边界截平，并有块状补边。
   - 要求：恢复自然卷发顶部轮廓，不得平切。
...
10. `steve_jobs.jpg`
    - 下方手部/衣服区域仍残留白色圆弧/圆环轮廓。
    - 要求：清除白色圆弧，不改变手和衣服主体。

11. `tim_cook.jpg`
    - 下方肩部/躯干周围仍有明显白色圆环/描边。
    - 要求：去掉白色圆环，使人物与背景直接连续。

### 第三次返工原则

- 只修上述 11 张。
- 不再对已经通过的 8 张做任何处理。
- 优先使用**当前人物像素 + 原始根目录旧图**进行非生成式修复。
- 对头发/头顶问题：禁止通过简单矩形填充制造平切；应从原图保留真实发型轮廓，或通过扩大画布/移动人物/背景延展解决。
- 对圆环问题：只移除圆环/描边，不要把人物轮廓一并抹掉。
- 最终必须同时满足：
  - 无圆环；
  - 无圆外角落；
  - 无矩形补块；
