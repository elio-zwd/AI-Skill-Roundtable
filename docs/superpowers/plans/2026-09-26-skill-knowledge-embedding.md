...
- 用户在本地双 lane 已运行至约 `1500/2972` pending 进度后，希望评估 4 路并发。远端在 `25edb5c5` 增加 `--concurrency 1|2|4`，默认仍为 2；4 路模式把账号 A 的 10 个 Project 自动拆为 5+5、账号 B 拆为 5+5，仍保持单个生成器进程、单 checkpoint 写入者，并以 0.25 秒 lane stagger 错开发起请求。该变更不影响已运行的旧 Python 进程；切换前必须先停止旧生成器并保留 checkpoint，再拉取最新 Head。本地 4-lane Python 测试尚待执行。RPD 剩余量不是提速依据；实际吞吐仍受每 Project RPM/TPM/RPD 任一维度约束。
