...
- [ ] 用户确认最终视觉方向；
- [ ] 将最终视觉决定同步进决策记录与品牌规格；
- [ ] 完成正式 Logo、App Icon、字体、商标与相似度的对应核验。

以上事项不阻止 PR08-F 批准与合并，也不阻止已授权的 PR09-01～15；未全部满足前不得启动 PR09-16。
...
- [x] 最终视觉比较方法已建立；
- [x] Head `03e995193b86ef4e43c1b048dd3c2c7ea0d835fa` 完成远端 CI、本地静态验收和独立只读复核，复核结论为 `PASS WITH REQUIRED CHANGES`；
- [ ] Required Changes 已在新的精确 Head 修正并通过远端 CI 与本地只读验收；
- [ ] 另一个 AI 对修正后的最新精确 Head 完成无阻塞严格只读复核；
- [ ] 用户在独立复核后明确批准 PR08-F；
