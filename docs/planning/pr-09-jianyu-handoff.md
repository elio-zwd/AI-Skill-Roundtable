...
- PR09-06 直接消费 04、05、07、09，不依赖 10A / 10B；
- PR09-08 直接消费 05、06、07、09，其中 06 提供已确认议题进入工作区的集成入口；
- PR09-10A 直接消费 03、07、09，通过 06 的工作区集成点接入，但不依赖 08 的交叉讨论行为；
- PR09-10B 直接消费 02B、10A，并向 12 提供音频/附件删除、恢复和清理接口；
- PR09-12 直接消费 03、07、10A、10B、11，拥有统一删除入口，但不得重写 10B 音频状态机。

除上述直接依赖外，箭头还表达串行集成顺序；不得仅因两个阶段相邻就臆造额外产品依赖。
