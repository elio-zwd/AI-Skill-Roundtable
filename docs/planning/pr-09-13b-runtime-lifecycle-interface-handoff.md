...
- `Unavailable`：重新打开数据库失败，不允许回退到旧 Runtime。
...
只有 `afterReopen` 完成后，Provider 才发布新的 `Ready` 世代。
...
| `WHILE_CLOSED` | 保留实际备份阶段的稳定错误，如 `target_write_failed`、`source_changed`、`operation_canceled` |
| `REOPEN` | `verification_failed`，同时 App 进入 `Unavailable` |
| `AFTER_REOPEN` | `database_integrity_failed` 或 `verification_failed` |

认证、KDF、SAF 等错误仍按 PR09-13A 冻结错误模型映射，不由 Runtime Lifecycle 层决定。

## 7. 取消语义

- 在租约归零前取消：恢复原 `Ready`，不关闭数据库；
- 在 `beforeClose` 中取消：恢复原 `Ready`，不关闭数据库；
- 在数据库关闭后取消：先在 `NonCancellable` 收尾区尝试重开，再传播取消；
- 重开失败：发布 `Unavailable`；
- PR09-13B 必须把用户取消映射为 `operation_canceled`；
- 取消不得被映射为成功，不得保留可识别为有效的最终 Snapshot。
...
普通业务写和正式 Worker 在 PR09-13B 中还必须接入 `BackupOperationGate` 读锁；Runtime 租约不能替代该跨进程/进程内写门禁。
...
8. External Process Recovery 两阶段继续通过；
9. Room Schema、Manifest 和系统备份 XML 无变化。

## 13. PR09-13B 禁止重新选择的决定

- Runtime Provider 是唯一数据库生命周期所有者；
- 使用世代和租约隔离旧对象图；
- Compose 世代切换清理旧 ViewModelStore；
- 闭库后的重开位于不可取消收尾区；
- 重开失败进入 `Unavailable`，不复活旧 Runtime；
- 不使用第二套 Room Builder；
