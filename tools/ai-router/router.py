#!/usr/bin/env python3
"""见域本地 AI Router（当前不使用，保留历史实验参考）。

本模块不属于见域当前正式开发/验收流程，也不是“网页版 GPT ↔ 本地 AI”的现行协作通道。
除非未来经过新的明确设计、评审和启用授权，否则不要运行或配置真实 Drive/OAuth/浏览器发送。

实现本身是一个只读 Drive watcher 与事件路由器：它不会修改 Drive 控制文件、生产代码或 Git
