...
class ResourceLifecycleSchemaContractTest {
    ...
    fun schemaKeepsDraftArtifactAudioAndLifecycleBoundariesSeparate() {
        val schema = findSchemaFile().readText()
        val officialMembersCreateSql = schema.createSqlFor("official_skill_combination_members")
        val lifecycleCreateSql = schema.createSqlFor("issue_lifecycle")
        val draftCreateSql = schema.createSqlFor("stage_summary_drafts")
        val audioCreateSql = schema.createSqlFor("audio_assets")

        assertFalse(
            "官方组合成员不得保存 System Prompt",
            officialMembersCreateSql.contains("systemPrompt", ignoreCase = true)
        )
        assertFalse(
            "官方组合成员不得保存自定义 Prompt 正文",
            officialMembersCreateSql.contains("customPrompt", ignoreCase = true)
        )
        assertFalse(
            "草稿不得带自动过期字段",
            draftCreateSql.contains("expiresAt", ignoreCase = true)
        )
        assertFalse(
            "生命周期不得带自动过期字段",
            lifecycleCreateSql.contains("expiresAt", ignoreCase = true)
        )
        assertFalse(
            "生命周期不得带自动清空字段",
            lifecycleCreateSql.contains("autoPurge", ignoreCase = true)
        )
        assertTrue(
            "生命周期必须表达 active 默认状态",
            lifecycleCreateSql.contains("DEFAULT 'active'")
        )
        assertTrue(
            "音频资产必须保留缺失文件状态扩展字段",
            audioCreateSql.contains("fileState")
        )
        assertTrue(
            "音频资产必须同时支持消息或成果来源",
            audioCreateSql.contains("sourceMessageId") &&
                audioCreateSql.contains("sourceArtifactId")
        )
        assertFalse(
    ...
    private fun String.createSqlFor(tableName: String): String {
        val entityPattern = Regex(
            """\{\s*"tableName"\s*:\s*"${Regex.escape(tableName)}".*?"createSql"\s*:\s*"(.*?)"\s*,\s*"fields""" ,
            setOf(RegexOption.DOT_MATCHES_ALL)
        )
        return entityPattern.find(this)?.groupValues?.get(1)
            ?: error("Schema 缺少 $tableName.createSql")
    }
