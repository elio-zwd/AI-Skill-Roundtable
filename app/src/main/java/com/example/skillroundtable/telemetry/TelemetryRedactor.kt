...
object TelemetryRedactor {
    ...
    private val queryKeyPattern = Regex("(?i)([?&](?:key|api_key|access_token)=)[^&\\s]+")
    ...
    fun stripUrlQuery(url: String): String {
        ...
        return if (questionMark >= 0) url.substring(0, questionMark) + "?[REDACTED_QUERY]" else url
    }
}

fun truncateTelemetryText(input: String, maxChars: Int): String {
    if (input.length <= maxChars) return input
    return input.take(maxChars) + "…[已截断，原始长度 ${input.length}]"
}
