...
object OfficialSkillCatalogParser {
     ...
     * 不允许复制或扩展第二份 Catalog。v1 保留首批限制，v2 必须完整发布 44 项。
     */
    fun parse(
        source: String,
    ...
    fun validate(manifest: OfficialSkillCatalogManifest): List<OfficialSkillCatalogValidationIssue> {
        ...
        manifest.skills.forEach { skill ->
            ...
            if (skill.domainTags.isEmpty()) {
                issues += issue("missing_domain", "至少需要一个领域标签", skill.id)
            }
            if (skill.scenarioTags.isEmpty()) {
                issues += issue("missing_scenario", "至少需要一个场景标签", skill.id)
            }
            if (skill.inputTags.isEmpty()) {
                issues += issue("missing_input", "至少需要一个输入标签", skill.id)
            }
            if (skill.outputTags.isEmpty()) {
                issues += issue("missing_output", "至少需要一个输出标签", skill.id)
            }
            ...
            if (skill.availability.executable) {
                ...
                if (
                    skill.sourceStatus !=
                    OfficialSkillSourceStatus.VERIFIED_IMPLEMENTATION_SOURCE
                ) {
                    issues += issue(
    ...
    private fun validateV1ExecutionPublication(
        ...
        if (publication.schemaVersion != EXPECTED_V1_EXECUTION_SCHEMA_VERSION) {
            issues += issue("invalid_execution_schema_version", "首批执行发布仅支持 schemaVersion=1")
        }
        ...
        issues += validatePublicationEntries(
    ...
    private fun validateV2ExecutionPublication(
        ...
        if (publication.schemaVersion != EXPECTED_V2_EXECUTION_SCHEMA_VERSION) {
            issues += issue(
                ...
                "全量执行 Manifest 仅支持 schemaVersion=2",
            )
        }
        if (publication.generatedFrom.isBlank()) {
            issues += issue("missing_execution_generation_source", "执行发布必须记录原创实现来源")
        ...
        val missing = baseIds.toSet() - publicationIds.toSet()
        missing.sorted().forEach { id ->
            issues += issue("missing_execution_skill", "全量执行 Manifest 缺少固定目录 Skill", id)
        }
        ...
        publication.skills.forEach { entry ->
            ...
            if (
                ...
                val combined = entry.boundaries.joinToString(" ")
                if (entry.boundaries.size < 4 || !combined.contains("专业复核")) {
                    issues += issue(
                        "missing_high_stakes_execution_boundary",
                        "高后果执行发布必须包含专业复核等完整边界",
                        entry.id,
                    )
                }
            }
            when (entry.id) {
                ...
                "culture-fortune-entertainment" -> {
                    if (entry.boundaries.none { it.contains("重大决策") }) {
                        issues += issue(
                ...
                "office-document-productivity" -> {
                    val combined = entry.boundaries.joinToString(" ")
                    if (!combined.contains("Markdown") || !combined.contains("不控制桌面")) {
                        issues += issue(
                ...
                "original-expression-naturalizer" -> {
                    if (entry.integrityBoundaries.size < 6) {
                        issues += issue(
                            "missing_integrity_boundaries",
                            "自然表达优化必须包含完整诚信边界",
                            entry.id,
